...
def save_sac_agent(agent, path='sac_model'):
    """保存 SAC 智能体的所有必要组件"""
    # en: Save all necessary components of SAC agent
    torch.save({
...
def load_sac_agent(agent, path='sac_model', device='cuda'):
    """加载 SAC 智能体的所有必要组件"""
    # en: Load all necessary components of SAC agent
    ...
    # agent.target_entropy = checkpoint['target_entropy']
...
def save_training_history(return_list, return_list_2, path='training_history'):
    """保存训练过程中的回报列表"""
    # en: Save the list of returns during training
    history = {
...
def load_training_history(path='training_history'):
    """加载训练过程中的回报列表"""
    # en: Load the list of returns during training
