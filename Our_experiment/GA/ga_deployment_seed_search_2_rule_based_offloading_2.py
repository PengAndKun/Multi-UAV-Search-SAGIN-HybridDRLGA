...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Trajectory actions are generated by a simple greedy policy using only local observation uncertainty; "
            "offloading actions use SAC offload agent."
        )
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "ga_best_deployment_simplegreedy_offload_random{sample_size}_w{wind_seed}_g{terrain_seed}_i{infra_seed}"
...
def choose_simple_greedy_actions(state, env, rng):
    """
    Simple greedy trajectory policy:
    - use only each UAV's local observation (uncertainty channel),
    - choose the direction with maximum uncertainty in local 15x15 window,
    - random tie-break among max-valued actions.
    """
    if state is None:
        return [0 for _ in range(env.N)]

    actions = []
    center_x = int(state.shape[-2] // 2)
    ...
    for i, uav in enumerate(env.uavs):
        ...
        for action_idx, direction in enumerate(env.directions):
            ...
            if nx < 0 or nx >= local_uncertainty.shape[0] or ny < 0 or ny >= local_uncertainty.shape[1]:
                continue

            value = float(local_uncertainty[nx, ny])
            if value < 0:
                # -1 is used in observation for invalid/out-of-grid/occupied cells.
                continue
...
def trajectory_execution_simple_greedy_offloading(
    env,
    offload_agent,
    position_starts,
    position_ends,
    ...
    greedy_rng = np.random.default_rng(rollout_seed)

    while not done:
        step_count += 1

        actions = choose_simple_greedy_actions(state=state, env=env, rng=greedy_rng)
...
def evaluate_chromosome(
    ...
    for seed_idx, traj_seed in enumerate(traj_seeds):
        ...
        for rep in range(repetitions):
            ...
            rollout_result = trajectory_execution_simple_greedy_offloading(
                env=env,
...
def main():
    ...
    result_data = {
        ...
        "mode": "simple_greedy_local_observation_with_offloading_policy",
        "seeds": {
    ...
    print("-" * 90)
    print("GA deployment search (random traj seeds, simple greedy trajectory + offloading policy) completed.")
