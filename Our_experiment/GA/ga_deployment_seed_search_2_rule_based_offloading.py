...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Trajectory actions are generated by a rule-based policy; offloading actions use SAC offload agent."
        )
    )
    ...
    parser.add_argument(
        ...
        default=(
            ...
            "ga_best_deployment_ruletraj_offload_random{sample_size}_w{wind_seed}_g{terrain_seed}_i{infra_seed}"
...
def choose_rule_based_actions(env, rng, previous_actions=None):
    """
    Rule-based trajectory policy:
    - prioritize high-uncertainty cells,
    - avoid collisions with other UAVs,
    - penalize high wind-energy movement,
    - bias toward destination when energy is relatively low.
    """
    actions = []
    reserved_next_positions = set()
    ...
    for i, uav in enumerate(env.uavs):
        ...
        cur_pos = np.array(uav["position"], dtype=np.int64)
        dest_pos = np.array(uav["destination"], dtype=np.int64)
        cur_xy = (int(cur_pos[0]), int(cur_pos[1]))
        wind_vector = np.array([env.wind_u[cur_xy[0], cur_xy[1]], env.wind_v[cur_xy[0], cur_xy[1]]], dtype=np.float64)

        energy_ratio = float(uav["energy"]) / float(env.initial_energy) if float(env.initial_energy) > 0 else 1.0
        low_energy_factor = max(0.0, (0.35 - energy_ratio) / 0.35)

        best_score = -1e18
        best_actions = []

        for action_idx, direction in enumerate(env.directions):
            direction_vec = np.array(direction, dtype=np.int64)
            next_pos = cur_pos + direction_vec
            nx, ny = int(next_pos[0]), int(next_pos[1])

            if nx < 0 or nx >= env.Lx or ny < 0 or ny >= env.Ly:
                continue

            score = 0.0

            # Encourage discovering high-uncertainty cells (typically 1 for unseen area).
            score += 2.0 * float(env.uncertainty_matrix[nx, ny])

            # Penalize selecting cells already chosen in this step by previous UAVs.
            if (nx, ny) in reserved_next_positions:
                score -= 2.5

            # Penalize moving into current cells of other active UAVs.
            for j, other in enumerate(env.uavs):
                if j == i or bool(other["done"]):
                    continue
                if (nx, ny) == tuple(other["position"]):
                    score -= 1.5
                    break

            # Penalize high flight energy under local wind.
            real_speed = float(env._get_real_v(direction_vec, wind_vector))
            energy_cost = 0.5 * float(env.M) * (real_speed ** 2) * float(env.T)
            score -= 1e-4 * energy_cost

            # If battery gets low, bias trajectory toward destination.
            score -= low_energy_factor * 0.9 * _distance((nx, ny), dest_pos)

            # Small inertia to reduce jitter.
            if previous_actions is not None and i < len(previous_actions) and int(previous_actions[i]) == int(action_idx):
                score += 0.05

            if score > best_score + 1e-12:
                best_score = score
                best_actions = [int(action_idx)]
            elif abs(score - best_score) <= 1e-12:
                best_actions.append(int(action_idx))

        if len(best_actions) == 0:
            chosen_action = 0
        elif len(best_actions) == 1:
            chosen_action = int(best_actions[0])
        else:
            chosen_action = int(rng.choice(np.array(best_actions, dtype=np.int64)))

        actions.append(chosen_action)

        direction_vec = np.array(env.directions[chosen_action], dtype=np.int64)
        next_pos = cur_pos + direction_vec
        nx, ny = int(next_pos[0]), int(next_pos[1])
        if 0 <= nx < env.Lx and 0 <= ny < env.Ly:
            reserved_next_positions.add((nx, ny))

    return actions


def trajectory_execution_rule_based_offloading(
    env,
    offload_agent,
    position_starts,
    position_ends,
    ...
    prev_actions = None
    rule_rng = np.random.default_rng(rollout_seed)

    while not done:
        step_count += 1

        actions = choose_rule_based_actions(env=env, rng=rule_rng, previous_actions=prev_actions)
        prev_actions = list(actions)
        trajectory_actions.append(actions)
        _, _, done_move = env.step(actions)
    ...
    return {
        ...
        "offloading_actions": offloading_actions,
...
def evaluate_chromosome(
    ...
    for seed_idx, traj_seed in enumerate(traj_seeds):
        ...
        for rep in range(repetitions):
            ...
            rollout_result = trajectory_execution_rule_based_offloading(
                env=env,
            ...
            _offload_actions = list(rollout_result["offloading_actions"])
...
def main():
    ...
    env, _, offload_agent = build_agents_and_env(num_uav=args.num_uav)
    ...
    result_data = {
        ...
        "mode": "rule_based_trajectory_with_offloading_policy",
        "seeds": {
    ...
    print("-" * 90)
    print("GA deployment search (random traj seeds, rule-based trajectory + offloading policy) completed.")
