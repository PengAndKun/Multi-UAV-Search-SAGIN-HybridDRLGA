...
def visualize_trajectory(
    ...
    def draw_uncertainty_matrix(uncertainty_matrix):
        ...
        for i in range(GRID_SIZE):
            for j in range(GRID_SIZE):
                ...
                # 根据不确定度大小设置颜色深浅 en: Set color based on uncertainty
    ...
    def draw_wind_field(wind_u, wind_v):
        """绘制风场向量"""
        scale = 5.0  # 缩放因子，调整箭头长度  en: Scale factor for arrow length
    ...
    def draw_offload_ratio(unload_count):
        ...
        # 绘制卸载比例 信息 en: Draw offload ratio information
    ...
    def simulate_trajectory(seed=0):
        ...
        while not done:
            ...
            # 从训练好的智能体获取动作 # Get actions from the trained agents
            actions = [agent.take_action(state[n]) for n in range(env.N)]
                    
            # 在环境中执行步骤 # Take a step in the environment with the actions
            next_state, reward, done = env.step(actions)
            
            # 额外的卸载动作决策 # Additional offload action decision
            offload_data = env.get_obs_2()  # 状态是一个图数据 # Offload data is a graph data structure
            ...
            for i, uav in enumerate(env.uavs):
                # 只有未结束的无人机才会被计入访问统计 # Only UAVs that are not done will be counted in visit statistics
                ...
                if len(trajectories[i]) > 0:
                    ...
                    if prev_pos != current_pos:
                        # 为了确保路径键的唯一性，无论方向如何，总是将较小坐标放在前面 en: Ensure path key uniqueness by sorting coordinates
                ...
                # 记录当前卸载行为 # Record current offload action
                offload_action = offload_actions[i] if i < len(offload_actions) else 0
                unload_count[current_pos][offload_action] += 1
                offload_heatmap[grid_x, grid_y] += 1
                offload_heatmaps_by_target[offload_action, grid_x, grid_y] += 1
            
            # 清屏并绘制 内容 # Clear the screen and draw the content
            screen.fill(WHITE)
            draw_uncertainty_matrix(env.uncertainty_matrix)
            draw_grid()
            draw_wind_field(env.wind_u, env.wind_v)  # 绘制风场 en: Draw wind field
            # draw_visit_count(visit_count)  # 可选：不再显示访问次数，减少视觉干扰 en: Optional: Do not display visit counts to reduce visual clutter
            ...
            for i, traj in enumerate(trajectories):
                if len(traj) > 1:
                    for j in range(len(traj) - 1):
                        ...
                        # 计算该路径的经过次数并据此调整宽度 # Calculate the frequency of this path and adjust the width accordingly
                        path_key = tuple(sorted([start_pos, end_pos]))
                        path_frequency = path_count.get(path_key, 1)
                        width = min(3 + path_frequency, 5)  # 根据路径经过次数调整宽度 en: Adjust width based on path frequency
                        
                        pygame.draw.line(screen, colors[i % len(colors)], start_pos, end_pos, width)

                        # 添加方向箭头 # Add directional arrows
                        # 计算线段的位置点，使箭头更靠近终点 en:  Calculate the position of the line segment to place the arrow closer to the end point
        ...
        if return_stats:
            return {
                "avg_uncertainty": float(current_avg_uncertainty),
...
def visualize_trajectory_from_action(agent, offload_agent, env,position_starts,position_ends,trajectory_actions,offloading_actions,seed=0):
    # 初始化 Pygame en: Initialize Pygame
    pygame.init()
    ...
    BLUE = (0, 0, 255)
    GREEN = (0, 200, 0)  # 墨绿色 en: Dark green
    YELLOW = (255, 150, 42)  # 棕色 e
    PURPLE = (128, 0, 128)  # 添加紫色用于显示不确定度
    ...
    def draw_wind_field(wind_u, wind_v):
        """绘制风场向量"""
        scale = 5.0  # 缩放因子，调整箭头长度
        for i in range(0, GRID_SIZE, 1):  # 每隔2个格子绘制一个箭头，避免过于密集
            for j in range(0, GRID_SIZE, 1):
                u = wind_u[j, i]
                v = wind_v[j, i]
                # 计算风向箭头的起点和终点
                ...
                # 绘制箭头线
                if abs(u) > 0.1 or abs(v) > 0.1:  # 只绘制足够大的风向
                    pygame.draw.line(screen, BLACK, (start_x, start_y), (end_x, end_y), 1)
                    # 计算箭头角度
                    angle = np.arctan2(v, u)
                    # 绘制箭头头部
        ...
        for (x, y), counts in unload_count.items():
            ...
            for i, count in enumerate(counts):
                if count > 0:
                    text = font.render(f"{OFFLOAD_TARGETS[i]}:{count}", True, OFFLOAD_COLORS[i])
                    screen.blit(text, (x - 30, y + text_y_offset))
                    text_y_offset += 15  # 向下移动以便显示下一个卸载信息
    ...
    def draw_avg_uncertainty(avg_uncertainty):
        ...
        # 绘制当前平均不确定度
        text_current = font.render(f"average uncertainty: {avg_uncertainty:.4f}", True, PURPLE)
        screen.blit(text_current, (SCREEN_WIDTH - 360, 20))
    ...
    def simulate_trajectory(position_starts,position_ends,trajectory_actions,offloading_actions,seed=0):
        """模拟并可视化无人机轨迹"""
        state = env.reset(seed,position_starts,position_ends)
        trajectories = [[] for _ in range(env.N)]  # 存储每个无人机的轨迹
        # 获取每个无人机的初始位置并添加到轨迹中
        for i in range(env.N):
            pos = env.uavs[i]['position']
            screen_x = int(pos[0] * CELL_SIZE + CELL_SIZE // 2)
        ...
        visit_count = np.zeros((GRID_SIZE, GRID_SIZE))  # 存储每个网格单元的访问次数
        unload_count = {}  # 存储每个位置的卸载次数，格式: {(x, y): [本地, 基站, HAPS, LEO, 云端]}
        ...
        while not done:
            ...
            if count >= len(offloading_actions):
                break
            # 从训练好的智能体获取动作
            actions = [trajectory_actions[count][n] for n in range(env.N)]
            ...
            offload_actions = [offloading_actions[count][n] for n in range(env.N)]
            next_offload_data, offload_reward, done = env.step_offload(offload_actions)
            count += 1
            ...
            for i, traj in enumerate(trajectories):
                if len(traj) > 1:
                    for j in range(len(traj) - 1):
                        ...
                        # 添加方向箭头
                        # 计算线段的位置点，使箭头更靠近终点
                        # 位置从中点(0.5)调整到靠近终点(0.7)的位置
                        pos_ratio = 0.8  # 箭头位置比例，0.5是中点，越大越靠近终点
                        arrow_x = start_pos[0] + (end_pos[0] - start_pos[0]) * pos_ratio
                        arrow_y = start_pos[1] + (end_pos[1] - start_pos[1]) * pos_ratio
        ...
        pygame.time.wait(15000)  # 显示最终结果3秒
        pygame.quit()

        return current_avg_uncertainty

    # 运行模拟
    return simulate_trajectory(position_starts,position_ends,trajectory_actions,offloading_actions,seed)
